...
> 历史文档：`docs/历史研究归档/部署文档/` 下的旧 DEPLOYMENT_*.md / FIX_SUMMARY.md 仅作为问题排查参考，**不再作为部署流程依据**。
...
打印所有 ssh / rsync / pm2 命令但不执行。**每次正式部署前必须先 dry-run 一次**；首次部署或目标路径变化时尤其不能跳过。
...
| 管理端 | `https://admin.xs.momofx.cn/` | 登录 → 看板 / 设置 / 数据统计正常 |
...
**管理站 `admin.xs.momofx.cn`**（推荐独立子域名）：
...
配置完后：
1. DNS 加 `admin.xs.momofx.cn` A 记录指向服务器 IP
...
脚本只在 `admin` 账号不存在时创建账号，并生成 20 位随机密码写入
`admin-backend/.admin-credentials`。如果 `admin` 已存在，脚本不会重置密码，
避免误改生产凭证。

首次部署后，运维必须 SSH 到服务器读取 `.admin-credentials`，妥善记录后立刻销毁该文件。
生产环境不得使用固定默认密码，也不要把管理员明文密码写进代码、文档或提交记录。
...
> 已归档到 `docs/历史研究归档/部署文档/`：

- `2025-11-01-DEPLOYMENT_COMPLETE.md` — 早期 V1.0 部署完成验收（2025-11-01）
- `2026-05-18-DEPLOYMENT_ISSUES_AND_SOLUTIONS.md` — 1290 行的问题汇编，遇到具体报错可全文搜索
- `2025-11-01-FINAL_DEPLOYMENT_STEPS.md` — 早期最后一公里步骤记录
- `2026-05-18-FIX_SUMMARY.md` — 历次部署修复总结
