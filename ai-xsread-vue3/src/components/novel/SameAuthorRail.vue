...
import { listAuthorNovels } from '@/api/authors'
...
watch(
  () => [props.authorId, props.excludeId],
  async () => {
    books.value = []
    if (!props.authorId) return
    try {
      const res = await listAuthorNovels(props.authorId, { pageSize: 8 })
  ...
  <section v-if="books.length">
    ...
    <div class="flex gap-3 overflow-x-auto no-scrollbar -mx-4 sm:-mx-6 lg:mx-0 px-4 sm:px-6 lg:px-0 pb-1">
      <RouterLink v-for="book in books" :key="book.id" :to="`/novel/${book.id}`" class="shrink-0 w-24 sm:w-28">
        ...
        <p v-if="book.author" class="text-[10px] text-ink-500 dark:text-ink-300">{{ book.author }}</p>
      </RouterLink>
    </div>
  </section>
</template>
